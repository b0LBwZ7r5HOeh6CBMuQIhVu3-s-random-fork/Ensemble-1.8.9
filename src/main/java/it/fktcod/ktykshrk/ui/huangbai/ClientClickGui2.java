...
public class ClientClickGui2 extends GuiScreen {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		TTFFontRenderer simpleton=Core.fontManager.getFont("SFM 6");
		...
		Core.fontManager.getFont("LOGO 14").drawString("Ensemble", (int) (startX + 10), (int) startY + 15,
				new Color(180, 180, 180, alpha).getRGB());
		Core.fontManager.getFont("LOGO 14").drawString(Core.VERSION, (int) startX +50, (int) startY + 15,
				new Color(180, 180, 180, alpha).getRGB());
		Core.fontManager.getFont("LOGO 14").drawString(
				"Hello, " + Core.UN + " !",
				(int) (startX + 430 - 50
						-Core.fontManager.getFont("LOGO 14").getWidth(
								"Hello, " + Minecraft.getMinecraft().getSession().getPlayerID() + " !")),
				(int) startY + 15, new Color(200, 200, 200, alpha).getRGB());
		...
		simpleton.drawString(currentModule.getDescription(), (int) startX + 200, (int) mY + 36,
		...
		for (int i = 0; i < currentModule.getValues().size(); i++) {
			...
			if (value instanceof NumberValue) {
				...
				sigmaFont.drawString(value.getValue().toString(),
						(int) startX + 300
								- (int) sigmaFont.getWidth(value.getValue().toString()),
			...
			if (value instanceof ModeValue) {
				...
				if (((ModeValue) value).getSelectMode() != null) {
					sigmaFont.drawStringWithShadow(((ModeValue) value).getSelectMode().getName(),
							(float) (x + 40
									- sigmaFont
											.getWidth(((ModeValue) value).getSelectMode().getName()) / 2),
...
//				startY = sr.getScaledHeight() / 2 - 350 / 2;
		...
		for (int i = 0; i < HackCategory.values().length; i++) {
			...
			sigmaFont.drawStringWithShadow(
