...
public class Profiler extends Module{
	...
	public Profiler() {
		...
		this.setChinese(Core.Translate_CN[75]);
	}
	...
	public void onRenderWorldLast(RenderWorldLastEvent event) {
		for (Object object : Utils.getEntityList()) {
			if(object instanceof EntityLivingBase) {
            	...
            	double xPos = (entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * event.partialTicks) - renderPosX;
            	double yPos = (entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * event.partialTicks) - renderPosY;
            	double zPos = (entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * event.partialTicks) - renderPosZ;
	...
	void renderNameTag(EntityLivingBase entity, String tag, double x, double y, double z) {
    	...
    	float distance = player.getDistanceToEntity(entity) / 4.0F;
    	if (distance < 1.6F) {
    		distance = 1.6F;
 	    }
     ...
     RenderUtils.drawRect(-width - 2, -(fontRenderer.FONT_HEIGHT + 1), width + 2, 2.0F, ColorUtils.color(0.0F, 0.0F, 0.0F, 0.0F));
//	     
...
//	     RenderUtils.drawRect(width + 3.0F, -(fontRenderer.FONT_HEIGHT + 1) - 1.0F, width + 2, 3.0F, color);
	     fontRenderer.drawString(tag, MathUtils.getMiddle(-width - 2, width + 2) - width, -(fontRenderer.FONT_HEIGHT - 1), Color.RED.getRGB(), true);
